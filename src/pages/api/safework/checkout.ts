...
import { config, json, validateInput, token, digest, stripe, PRODUCT, rateLimit } from '../../../lib/safework/orders.mjs';
export const prerender = false;
export const GET: APIRoute = ({locals}) => { const c = config(locals.runtime.env); return json({enabled:c.enabled, mode:c.mode, amount:Number.isSafeInteger(c.amount) ? c.amount : null}); };
export const POST: APIRoute = async ({request,locals}) => {
 ...
 try {
  ...
  await db.prepare('INSERT INTO safework_orders(id,token_hash,input_json,amount,mode,created_at) VALUES(?,?,?,?,?,?)').bind(id,await digest(access),JSON.stringify(input),c.amount,c.mode,Date.now()).run();
  const origin=new URL(request.url).origin;
  const returnUrl=`${origin}/safework/order?id=${id}#${access}`;
  const session=await stripe(c.key,'checkout/sessions',new URLSearchParams({mode:'payment',success_url:returnUrl,cancel_url:returnUrl, 'payment_method_types[0]':'card','line_items[0][quantity]':'1','line_items[0][price_data][currency]':'gbp','line_items[0][price_data][unit_amount]':String(c.amount),'line_items[0][price_data][product_data][name]':'SafeWork AI draft work pack','line_items[0][price_data][product_data][description]':'One job-specific draft work pack with repeat downloads. Competent-person review required.', 'metadata[product]':PRODUCT,'metadata[order_id]':id}),`safework-${id}`);
