...
export async function generatePack(request: Request, apiKey: string, model: string) {
  ...
  if (description.length > 12000) return Response.json({ message: 'The job description must be 12,000 characters or fewer.' }, { status: 400 });
...
Preserve confirmed facts from the user description throughout the pack: workforce number, materials, tools, occupancy, access restrictions and forecast weather. Populate workforce with the explicitly supplied number and roles; if absent, use "Not specified — confirm workforce and supervision before starting". Do not replace supplied facts with unknowns, assume extra workers, or add unrelated methods such as metal grinding to a timber-only job. Assess whether the stated team can maintain access control while handling materials; where inadequate, create a hold point instead of inventing extra staff.
...
Anything that cannot safely be known from the two supplied fields must be recorded as an explicit assumption and a PRE-START CONFIRMATION / HOLD POINT. Never invent dimensions, service clearance, ground conditions, asbestos status, isolation, competence, permits, design approval, plant selection, substances, access arrangements or welfare. A generated permit is never an issued permit. Apply the hierarchy of control; PPE must not replace higher-order controls. Risk ratings are provisional and must state the assumed scoring basis in plain language. Cite only relevant official sources contained in the supplied knowledge. The pack must require competent-person review, amendment for actual site conditions, workforce briefing and authorisation before work begins.
